...
tutorial_path = [matlabVisRoot ''];%这是一个变量加常数字符串组成的字符串数组，中间没有空格，赋值给左边的变量
addpath( tutorial_path );
im_path = [tutorial_path,'images/'];%tutorial_path是程序路径，im_path是图像的存储路径
...
im1 = pgmRead( [im_path,'einstein.pgm'] )./255;%转换所有象素的灰度值在[0,1]之间,haven't found the function "pgmRead"
[ pos1 scale1 orient1 desc1 ] = SIFT( im1, octaves, intervals, ones(size(im1)), 0.02, 5.0, 2 );
