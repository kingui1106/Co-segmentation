...
void BPFlow::LoadMasks(double* pMask1, double* pMask2)
{
    useMask = 1;
    
    pMask_s = new double[Width * Height];
    pMask_d = new double[Width2 * Height2];
    
	for(size_t j=0;j<Width*Height;j++)
        pMask_s[j]=pMask1[j];
    
    for(size_t j=0;j<Width2*Height2;j++)
        pMask_d[j]=pMask2[j];
}
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
                    ...
                    if(useMask)
                    {
					 foo = pMask_s[index] * pMask_d[index2] * foo + pMask_s[index] * (1-pMask_d[index2]) * KKnotK;

                       // foo = foo;
                        //foo = (1-pMask_s[index]) * (1-pMask_d[index2]) * foo;

                        //if( pMask_s[index]== 0 || pMask_d[index2]==0)//||
                          //  foo = KKnotK;
				...
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					if(!InsideImage(x,y))
                    {
						...
						pDataTerm[index][_ptr]=DefaultMatchingScore;
                         ...
                         if(useMask)// && pMask_s[index]== 0 )
                             pDataTerm[index][_ptr]= KKnotK;
                    }
